...
class RequestCommand extends ContainerAwareCommand
{
            ...
            $promise = $client->requestAsync('GET', $url->getName(), [
                ...
                'allow_redirects' => [
                    'on_redirect' => function (
                        ...
                        UriInterface $uri) use (&$url, $doctrine) {

                        $redirectedTo = new Url($uri->__toString(), $url->getBatch(), 'Still working');

                        $url->setStatus($response->getStatusCode());
                        $url->setChild($redirectedTo);
     ...
     * @return Url
     */
    private function getCurrentUrl(Url $url)
    {
        $newUrl = $url;

        while ($newUrl->getChild()) {
            $newUrl = $newUrl->getChild();
        }
